...
bool fdisreadable(fd_t fd){
	int flags;
	return IsOk(fdgetflags(fd,&flags)) && flags&O_RDONLY;
}

bool fdiswriteable(fd_t fd){
	int flags;
	return IsOk(fdgetflags(fd,&flags)) && flags&O_WRONLY;
}

bool fdisreadwriteable(fd_t fd){
	int flags;
	return IsOk(fdgetflags(fd,&flags)) && flags&O_RDONLY && flags&O_WRONLY;
